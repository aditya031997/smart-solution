...
import _, { isArray, map, result } from 'lodash';
...
const TransactionInvoice = () => {
    ...
    useEffect(() => {
        const updatedSelectedBorderData = selectedValues?.map((selectedId) => {
            ...
            return selectedBorder ? _.pick(selectedBorder, ['invoice_id', 'invoiceNumber', 'totalAmount', 'booking.invoice_status', 'booking.new_booking_id', 'booking']) : null;
        });

        setSelectedBorderData(updatedSelectedBorderData);
    }, [selectedValues, userData, filteredInvoices]);

    const overallTotalAmount = selectedBorderData.reduce((total, invoice) => total + invoice?.booking?.total_ammount, 0);
    ...
    console.log(selectedBorderData, 'filteredItems', userData);
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            const updatedInvoices = await Promise.all(
                selectedBorderData?.map(async (invoiceId) => {
                    try {
                        // Fetch the existing invoice data
                        const existingInvoice = await axios.get(`${config.API_BASE_URL}/invoice/${invoiceId?.invoice_id}`);

                        console.log(existingInvoice, 'existingInvoice', selectedBorderData);
                        // Update the total_amount property
                        existingInvoice.data.received = invoiceId?.booking?.total_ammount;
                        existingInvoice.data.isReceived = true;
                        // Make the API call to update the invoice
                        const updatedInvoice = await axios.put(`${config.API_BASE_URL}/invoice/${invoiceId?.invoice_id}`, existingInvoice.data);

                        return updatedInvoice.data;
                    } catch (error) {
                        console.error(`Error updating invoice ${invoiceId?.invoice_id}:`, error);
                        throw error;
                    }
                })
            );
    ...
    // const updatedInvoices = selectedBorderData.map(async (invoiceId) => {
    //     try {
    //         // Fetch the existing invoice data
    //         const existingInvoice = await axios.get(`${config.API_BASE_URL}/invoice/${invoiceId?.invoice_id}`);
    ...
    //         // Update the total_amount property
    //         existingInvoice.data.received = invoiceId?.totalAmount;
    ...
    // const updateInvoices = async (selectedBorderData) => {
    //     const updatedInvoices = await Promise.all(
    //         selectedBorderData?.map(async (invoiceId) => {
    //             try {
    //                 // Fetch the existing invoice data
    ...
    //                 // Update the total_amount property
    //                 existingInvoice.data.received = invoiceId?.booking?.total_ammount;
    //                 existingInvoice.data.isReceived = 1;
    //                 console.log(existingInvoice, 'existingInvoice', selectedBorderData);
                                                    ...
                                                    <select
                                                        ...
                                                        onChange={(e) => {
                                                            ...
                                                            const filtered = userData?.filter(
                                                                (invoice) => invoice?.customer_id == formik.values.customer && invoice?.booking?.invoice_status === 'generated' && !invoice?.isReceived
                                                            );
